...
class OrderService
{
    ...
    public function updateStatus(Order $order, string $newStatus, array $fields = [], bool $skipEmail = false): void
    {
        ...
        if ($skipEmail || !$order->customer_email) return;

        match($newStatus) {
            'confirmed' => $this->handleConfirmedEmail($order),
            'shipped'   => $this->sendShippedEmail($order),
            'cancelled' => $oldStatus !== 'cancelled' ? $this->handleCancellation($order) : null,
            default     => null,
        };
    }

    private function handleConfirmedEmail(Order $order): void
    {
        // Nëse kartë/bank dhe pagesa nuk është bërë ende — skip
        if (
            in_array($order->payment_method, ['bank', 'stripe']) &&
            empty($order->payment_intent_id)
        ) {
            Log::info("Email skip — pagesa me kartë nuk është konfirmuar: #{$order->order_number}");
            return;
        }
    ...
    public function sendConfirmedEmail(Order $order): void
    {
        try {
            Mail::to($order->customer_email)->send(
                new OrderConfirmed($order->load('items.product'))
            );
        } catch (\Exception $e) {
    ...
    public function sendShippedEmail(Order $order): void
    {
        try {
            Mail::to($order->customer_email)->send(
                new OrderShipped($order->load('items.product'))
            );
        } catch (\Exception $e) {
            Log::error('Email dërgimit dështoi: ' . $e->getMessage());
        }
    }
    ...
    public function statusMessage(string $status): string
    {
        return match($status) {
            ...
            'awaiting_payment' => 'Porosia është në pritje të pagesës.',
    ...
    private function handleCancellation(Order $order): void
    {
        ...
        try {
            Mail::to($order->customer_email)->send(
                new OrderCancelled($order->load('items.product'))
            );
        } catch (\Exception $e) {
